  ...
  d.assets.forEach(function(a){
    ...
    if(staticMedia&&staticMedia.type==='lottie'){
      try{
        ...
        la.addEventListener('complete',function(){la.goToAndPlay(0,true);});
...
RuntimeEngine.prototype.triggerMedia=function(mediaId){
  ...
  if(media.type==='lottie'){
    try{var ad=JSON.parse(atob(media.dataUrl.split(',')[1])); var la2=lottie.loadAnimation({container:el,renderer:'svg',loop:true,autoplay:true,animationData:ad}); la2.addEventListener('complete',function(){la2.goToAndPlay(0,true);});}catch(e){}
